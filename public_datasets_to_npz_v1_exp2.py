...
def getClassificationData(classification_dataset, plml_dataset = True):
    ...
    #Select only 100 balanced samples
    t_label = train_label.argmax(1)
    class0_idx = np.where(t_label == 0)[0]
    class1_idx = np.where(t_label == 1)[0]

    sel0_idx = np.random.choice(class0_idx, 50, replace = len(class0_idx) < 50)
    sel1_idx = np.random.choice(class1_idx, 50, replace = len(class1_idx) < 50)
    sel_idx = sel0_idx.tolist() + sel1_idx.tolist()

    train_ft = train_ft[sel_idx, :]
    train_label = train_label[sel_idx, :] 

    assert train_ft.shape[0] == 100, "Error! Wrong subsampling."
    assert train_label.shape[0] == 100, "Error! Wrong subsampling."


    t_label = train_label.argmax(1)
    t_classes, t_counts = np.unique(t_label, return_counts = True)
    assert t_counts.tolist() == [50, 50], "Error! Wrong subsampling."
...
def main():
    ...
    ##UCI-Datasets
    classification_dataset = "diabetes" #"dermatology" #"heart-disease" #"heart-disease-binary" #"diabetes" #"dermatology" #"heart-disease-binary" #"dermatology" #"cervical-cancer" #"ovarian-cancer" #"heart-disease-binary" #"heart-disease" 
    trial = "small5"
