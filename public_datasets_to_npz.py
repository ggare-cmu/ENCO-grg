...
def getClassificationData(classification_dataset, plml_dataset = True):
    ...
    for var, x in zip(vars_list, X.T):
        ...
        if var_unique == [0,1]:
            ...
            X_categorical.append(x)
            X_continous.append(x)
        elif len(var_unique) <= 20:
            var_type = 'categorical'

            x_cat = x.copy()
            cat_map_dict = dict(zip(var_unique, range(len(var_unique))))
            x_cat = [cat_map_dict[i] for i in x_cat]
    ...
    X_categorical = np.vstack(X_categorical).T
    X_continous = np.vstack(X_continous).T
    assert X.shape == X_categorical.shape, "Error! Type conversion wrong."
    assert X.shape == X_continous.shape, "Error! Type conversion wrong."
    ...
    test_sam_count = int(0.2 * num_samples)
    val_sam_count = int(0.1 * num_samples)
    samples_ids = np.arange(num_samples)
    test_ids = np.random.choice(samples_ids, test_sam_count, replace = False)
    samples_ids = [i for i in samples_ids if i not in test_ids]
    val_ids = np.random.choice(samples_ids, val_sam_count, replace = False)
    samples_ids = [i for i in samples_ids if i not in val_ids]

    assert len(samples_ids) + len(test_ids) + len(val_ids) == num_samples, "Error! Dataspliting done incorrectly."

    train_ft = X_categorical[samples_ids, :]
    train_ft_con = X_continous[samples_ids, :]
    train_label = y_one_hot[samples_ids, :]


    test_ft = X_categorical[test_ids, :]
    test_ft_con = X_continous[test_ids, :]
    test_label = y_one_hot[test_ids, :]

    val_ft = X_categorical[val_ids, :]
    val_ft_con = X_continous[val_ids, :]
    val_label = y_one_hot[val_ids, :]
    ...
    return train_ft, test_ft, val_ft, train_ft_con, test_ft_con, val_ft_con, train_label, test_label, val_label, class_names, num_classes, vars_list, class_list, num_input_features, feature_type
...
def main():
    ...
    ##UCI-Datasets
    classification_dataset = "heart-disease" #"breast_cancer_wisconsin" #"heart-disease" #"heart-disease-binary" #"diabetes" #"dermatology" #"heart-disease-binary" #"dermatology" #"cervical-cancer" #"ovarian-cancer" #"heart-disease-binary" #"heart-disease" 
    trial = "t5"

    plml_dataset = False

    train_ft, test_ft, val_ft, train_ft_con, test_ft_con, val_ft_con, train_label, test_label, val_label, class_names, num_classes, vars_list, class_list, num_input_features, feature_type = getClassificationData(classification_dataset, plml_dataset)
    ...
    if upsampleTrainFeatures:
        ...
        train_ft_con = train_ft_con[upsample_label_indices, :]
    ...
    train_biomarkers_con = np.concatenate((train_ft_con, train_label), axis = 1)
    assert train_biomarkers_con.shape == (train_ft_con.shape[0], train_ft_con.shape[1] + train_label.shape[1]), "Error! Features concatenation incorrect"


    test_biomarkers_con = np.concatenate((test_ft_con, test_label), axis = 1)
    assert test_biomarkers_con.shape == (test_ft_con.shape[0], test_ft_con.shape[1] + test_label.shape[1]), "Error! Features concatenation incorrect"


    val_biomarkers_con = np.concatenate((val_ft_con, val_label), axis = 1)
    assert val_biomarkers_con.shape == (val_ft_con.shape[0], val_ft_con.shape[1] + val_label.shape[1]), "Error! Features concatenation incorrect"
    ...
    train_biomarkers_con = train_biomarkers_con.astype(data_type)
    test_biomarkers_con = test_biomarkers_con.astype(data_type)
    val_biomarkers_con = val_biomarkers_con.astype(data_type)
    ...
    train_biomarkers_csv_con = np.concatenate((train_biomarkers_con[:,:-num_classes], train_biomarkers_con[:,-num_classes:].argmax(1)[:, np.newaxis]), axis = 1)
    np.savetxt(f"datasets/{classification_dataset}-{trial}_TrainUpsampledPatient_con.csv", train_biomarkers_csv_con, delimiter = ",")


    np.savez(f"datasets/{classification_dataset}-{trial}_TrainUpsampledPatient_con.npz", data_obs = train_biomarkers_con, data_int = data_int, adj_matrix = adj_matrix, 
            vars_list = vars_list, class_list = class_list, feature_type = feature_type)

    np.savez(f"datasets/{classification_dataset}-{trial}_TestUpsampledPatient_con.npz", data_obs = test_biomarkers_con, data_int = data_int, adj_matrix = adj_matrix, 
            vars_list = vars_list, class_list = class_list, feature_type = feature_type)

    np.savez(f"datasets/{classification_dataset}-{trial}_ValUpsampledPatient_con.npz", data_obs = val_biomarkers_con, data_int = data_int, adj_matrix = adj_matrix, 
            vars_list = vars_list, class_list = class_list, feature_type = feature_type)
