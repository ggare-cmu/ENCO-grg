...
def main():
    ...
    np.save(f"disease_modeldata2trainUpsampledPatient-{consolidation_type}.npy", train_biomarkers)

    # test_biomarkers = np.array(test_biomarkers, dtype=np.float64)
    test_biomarkers = np.array(test_biomarkers, dtype=np.float32)
    np.save(f"disease_modeldata3testUpsampledPatient-{consolidation_type}.npy", test_biomarkers)
    ...
    data_int = np.zeros((test_biomarkers.shape[1], 1, test_biomarkers.shape[1]), dtype=np.float32)
    ...
    np.savez(f"DiseaseModelBiomarkersGTabTrainUpsampledPatient-{consolidation_type}.npz", data_obs = train_biomarkers, data_int = data_int, adj_matrix = adj_matrix)
    ...
    np.savez(f"DiseaseModelBiomarkersGTabTestUpsampledPatient-{consolidation_type}.npz", data_obs = test_biomarkers, data_int = data_int, adj_matrix = adj_matrix)
