...
def fitLargeMLP(train_label_ft, test_label_ft, gt_train_scores, gt_test_scores, 
        n_trial = 3, hidden_layer_sizes = (128, 64, 32), verbose = True, Regression = False):
...
def predLargeMLP(clf, test_label_ft, Regression = False):
...
def train_generative_models(sorted_variables, node_parents, root_nodes, unconnected_nodes, train_data, category_classes, Regression = False, reports_path = "."):
    ...
    for var in sorted_variables:
        ...
        model, accuracy, ml_predictions, ml_prob_predictions = fitLargeMLP(
                                    ...
                                    verbose = False, Regression = Regression
                                )
...
def generateNewData(trained_models, sorted_variables, variables, node_parents, root_nodes, unconnected_nodes, 
            category_classes, num_samples = 1000, Binary_Features = True, Regression = False):
    ...
    for var in sorted_variables:
        ...
        model, ml_predictions, ml_prob_predictions = predLargeMLP(model, test_features, Regression)
...
def main():
    ...
    exp_name = "generateContinousSamples_500"
    
    results_dir = os.path.join(causal_discovery_exp_dir, exp_name)
    utils.createDirIfDoesntExists(results_dir)

    acyclic_adj_matrix_path = "binary_acyclic_matrix_001_DiseaseModelBiomarkersGTabTrainUpsampledPatient-max.npy"
    ...
    trained_gen_models = train_generative_models(sorted_variables, node_parents, root_nodes, unconnected_nodes, train_data, category_classes, Regression, results_dir)
    
    num_samples = 500 #200 #3000 #500 #1000
    Binary_Features = True #True
    synthetic_data = generateNewData(trained_gen_models, sorted_variables, variables, node_parents, root_nodes, unconnected_nodes, 
            category_classes, num_samples, Binary_Features, Regression)
