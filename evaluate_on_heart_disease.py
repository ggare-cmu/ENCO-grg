...
def main():
    ...
    # task = 'cifar-t1'
    # task = 'higgs_small-t1'
    task = 'heart-disease-binary-t5'

    trial = 'marco_AuC_T1'

    num_samples = 50 #60000 #10000 #50 #200 #3000 #500 #1000
    ...
    fake_data_path = f"/home/grg/Research/ENCO-grg/datasets/gan_fake_data/{fake_data}.csv"
    ...
    causal_discovery_exp_dir = f"/home/grg/Research/ENCO-grg/checkpoints/2022_26_Acyclic_{task}_TrainUpsampledPatient_GAN"
    ...
    train_data[:, :-num_classes] = (train_data[:, :-num_classes] - train_data[:, :-num_classes].min(0))/(train_data[:, :-num_classes].max(0) - train_data[:, :-num_classes].min(0))
    
    #Check for NaNs
    if np.any(np.isnan(train_data)):
        train_data = np.where(np.isnan(train_data), 0, train_data)

    assert train_data.dtype == large_synthetic_data.dtype, "Error! Data type mismatch."
    ...
    test_data[:, :-num_classes] = (test_data[:, :-num_classes] - test_data[:, :-num_classes].min(0))/(test_data[:, :-num_classes].max(0) - test_data[:, :-num_classes].min(0))
    
    #Check for NaNs
    if np.any(np.isnan(test_data)):
        test_data = np.where(np.isnan(test_data), 0, test_data)

    assert test_data.dtype == large_synthetic_data.dtype, "Error! Data type mismatch."
    ...
    val_data[:, :-num_classes] = (val_data[:, :-num_classes] - val_data[:, :-num_classes].min(0))/(val_data[:, :-num_classes].max(0) - val_data[:, :-num_classes].min(0))
    
    #Check for NaNs
    if np.any(np.isnan(val_data)):
        val_data = np.where(np.isnan(val_data), 0, val_data)

    assert val_data.dtype == large_synthetic_data.dtype, "Error! Data type mismatch."
