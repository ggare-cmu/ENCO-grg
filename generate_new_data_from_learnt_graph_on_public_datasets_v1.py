...
def getSamplingOrder(adj_matrix, variables, logger, results_dir):
...
def fitLargeMLP(train_label_ft, test_label_ft, gt_train_scores, gt_test_scores, label_classes,
...
def predLargeMLP(clf, test_label_ft, label_classes):
...
def train_generative_models(sorted_variables, node_parents, root_nodes, unconnected_nodes, train_data, category_classes, causal_feature_mapping, reports_path = "."):
...
def generateNewData(trained_models, sorted_variables, variables, node_parents, root_nodes, unconnected_nodes, 
            category_classes, num_samples = 1000, features_type = ''):
    ...
    for var in sorted_variables:
        ...
        if var in unconnected_nodes:
            ...
            if features_type == "binary":
                ...
                raise Exception(F"Error! Unsupported feature type = {features_type}")
        ...
        model, ml_predictions, ml_prob_predictions = predLargeMLP(model, test_features, label_class)
...
def main():
    ...
    # causal_discovery_exp_dir = "/home/grg/Research/ENCO/checkpoints/2022_23_Acyclic_DiseasePerPatient-max_GroundTruth_UpsampledTrainTest_sparity0.0004"
    # causal_discovery_exp_dir = "/home/grg/Research/ENCO/checkpoints/2022_24_Acyclic_DiseasePerPatient-max_modelContinousUpsampledTrainTest_v1"
    # causal_discovery_exp_dir = "/home/grg/Research/ENCO/checkpoints/2022_25_Acyclic_labor_TrainUpsampledPatient"
    # causal_discovery_exp_dir = "/home/grg/Research/ENCO/checkpoints/2022_26_Acyclic_parity5_TrainUpsampledPatient"
    causal_discovery_exp_dir = f"/home/grg/Research/ENCO/checkpoints/2022_26_Acyclic_{task}_TrainUpsampledPatient"


    num_samples = 75 #200 #3000 #500 #1000
    # Binary_Features = False #True
    features_type = 'categorical' #categorical, binary, continous
    ...
    train_data_path = f"{task}_TrainUpsampledPatient.npz"
    ...
    #Get sampling order
    G, sorted_variables, node_parents, root_nodes, unconnected_nodes = getSamplingOrder(adj_matrix, variables, logger, results_dir)


    ### Train Generative Models

    trained_gen_models = train_generative_models(sorted_variables, node_parents, root_nodes, unconnected_nodes, train_data, category_classes, causal_feature_mapping, results_dir)
    ...
    synthetic_data = generateNewData(trained_gen_models, sorted_variables, variables, node_parents, root_nodes, unconnected_nodes, 
            category_classes, num_samples, features_type)

    #Sasve the generated synthetic data
    np.save(os.path.join(results_dir, f"synthetic_data_{num_samples}.npz"), synthetic_data)
    ...
    test_data_path = f"{task}_TestUpsampledPatient.npz"
    ...
    ##Train model
    model, accuracy, ml_predictions, ml_prob_predictions = fitRandomForest(
                                train_label_ft =  train_features, test_label_ft = test_features, 
                                # gt_train_scores = train_labels, gt_test_scores = test_labels, 
                                gt_train_scores = train_labels.argmax(1), gt_test_scores = test_labels.argmax(1), 
                                label_classes = label_classes,
    ...
    logger.log(f"[RandomForest] Accuracy on original train set = {accuracy}")
    ...
    synthetic_model, synthetic_accuracy, synthetic_ml_predictions, synthetic_ml_prob_predictions = fitRandomForest(
                                train_label_ft =  synthetic_train_features, test_label_ft = test_features, 
                                # gt_train_scores = synthetic_train_labels, gt_test_scores = test_labels, 
                                gt_train_scores = synthetic_train_labels.argmax(1), gt_test_scores = test_labels.argmax(1), 
                                label_classes = label_classes,
    ...
    logger.log(f"[RandomForest] Accuracy on synthetic + original train set = {synthetic_accuracy}")
