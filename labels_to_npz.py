...
def main():
    ...
    np.savez("BiomarkersGTabc.npz", data_obs = biomarkers, data_int = data_int, adj_matrix = adj_matrix)


    train_biomarkers = np.array(train_biomarkers, dtype=np.uint8)
    np.save("data2train.npy", train_biomarkers)

    test_biomarkers = np.array(test_biomarkers, dtype=np.uint8)
    np.save("data3test.npy", test_biomarkers)

    # np.savez("BiomarkersGTTrain.npz", data_obs = train_biomarkers, data_int = data_int, adj_matrix = adj_matrix)
    np.savez("BiomarkersGTabcTrain.npz", data_obs = train_biomarkers, data_int = data_int, adj_matrix = adj_matrix)

    # np.savez("BiomarkersGTTest.npz", data_obs = test_biomarkers, data_int = data_int, adj_matrix = adj_matrix)
    np.savez("BiomarkersGTabcTest.npz", data_obs = test_biomarkers, data_int = data_int, adj_matrix = adj_matrix)
