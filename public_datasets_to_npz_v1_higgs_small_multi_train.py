...
def getClassificationData(classification_dataset):
    ...
    feature_type = {}
    for var, x, x_min, x_max in zip(vars_list, X.T, X_min, X_max):
...
def main():
    ...
    for idx, size in enumerate(sub_dataset):
        ...
        if len(class0_ids) >= cls_sz:
        
            #Select class-0 as its the minority class
            cur_class0_ids = np.random.choice(class0_ids, cls_sz - len(sel_class0_ids), replace = False)
        ...
        elif len(class0_ids) < cls_sz and len(class0_ids) > 0:
            
            sel_class0_ids.extend(class0_ids)
            class0_ids = []
        
        elif len(class0_ids) == 0:
            pass
        else:
        ...
        #Select remaining samples from class-1 as its the majority class
        cur_class1_ids = np.random.choice(class1_ids, size - len(sel_class0_ids) - len(sel_class1_ids), replace = False)
        # [class1_ids.remove(i) for i in cur_class0_ids]
        ...
        print(f"[Sel-{idx+1}] required size = {size} | Labels counts = {classes, count}")
