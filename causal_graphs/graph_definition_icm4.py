...
class CausalDAGDataset(CausalDAG):
    ...
    def __init__(self, adj_matrix, data_obs, data_int, latents=None, exclude_inters=None, vars_list = None, class_list = None, feature_type = None):
        """
        ...
        # new_data_obs = []
        # for var, x in zip(features_list, data_obs.T):
        
        #     var_unique = np.unique(x).tolist()
            
        #     var_type = feature_type[var]

        #     if var_type == 'binary':

        #         new_data_obs.append(x)
        ...
        # new_data_obs = np.vstack(new_data_obs).T
        # assert data_obs.shape == new_data_obs.shape, "Error! Type conversion wrong."
        ...
        if data_obs.dtype == np.int32:
            ...
            num_categs = data_obs.max(axis=0) #Bug-Fix GRG: Max should be taken along num_samples axis not num_vars axis
        ...
        if self.vars_list is not None and self.class_list is not None:
            self.features_list = self.vars_list.tolist() + self.class_list.tolist()
            
            self.node_names_mapping = {}

            for idx, var in enumerate(self.features_list):
                # self.node_names_mapping[f"$X_\\{{idx}\\}$"] = var
                self.node_names_mapping["$X_{" + str(idx+1) + "}$"] = var

            self.num_classes = len(self.class_list)

        # self.adj_matrix_mask = adj_matrix
        self.adj_matrix_mask = np.zeros_like(adj_matrix).astype(bool)
        self.adj_matrix_mask[-self.num_classes:, :] = True
