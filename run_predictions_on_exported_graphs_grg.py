...
from experiments.utils import set_seed, get_basic_parser, test_graph, pred_graph
    ...
    for gindex, graph_path in enumerate(args.graph_files):
        ...
        if graph.num_vars <= 100:
            ...
            visualize_graph(graph,
                            filename=os.path.join(checkpoint_dir, "pred_graph_%s.pdf" % (file_id)),
                            figsize=(figsize, figsize),
                            layout="circular" if graph.num_vars < 40 else "graphviz")
        s = "== Predicting using graph \"%s\" ==" % graph_name
        print("="*len(s)+"\n"+s+"\n"+"="*len(s))
        # Start structure learning
        pred_graph(graph, args, checkpoint_dir, file_id)
